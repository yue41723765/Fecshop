...
public class LoadExtraBuilder {
    ...
    private MethodSpec.Builder builderE,builderV;
    ...
    public LoadExtraBuilder(ParameterSpec parameterSpec) {
        ...
        builderE = MethodSpec.methodBuilder(Consts
                .METHOD_LOAD_INTENT)
                .addAnnotation(Override.class)
                .addModifiers(Modifier.PUBLIC)
                .addParameter(parameterSpec);
        builderV=  MethodSpec.methodBuilder(Consts
                .METHOD_LOAD_View)
                .addAnnotation(Override.class)
                .addModifiers(Modifier.PUBLIC)
                .addParameter(parameterSpec);
    ...
    public void buildFindViewByIdBody(Element element) {
        ...
        builderV.addStatement(statement, redId);
    }
    ...
    public void buildGetIntent(Element element) {
        TypeMirror typeMirror = element.asType();
        ...
        builderE.addStatement(statement, extraName);
    }
...
//            testService.test();
            ...
            builderE.addStatement(statement, TypeName.get(element.asType()), Consts.ROUTER,
                    extraName);
            return;
        } else {
            //List
        ...
        switch (typeMirror.toString()) {
            ...
            default:
                ...
                statement = "$T[] " + fieldName + " = t.getIntent()" +
                        ...
                        "($S)";
                builderE.addStatement(statement, parcelableType, extraName);
                builderE.beginControlFlow("if( null != $L)", fieldName);
                statement = defaultValue + " = new $T[" + fieldName + ".length]";
                builderE.addStatement(statement, arrayTypeName.componentType)
    ...
    public void injectTarget(ClassName className) {
        builderE.addStatement(INJECT_TARGET, className, className);
        builderV.addStatement(INJECT_TARGET, className, className);
    }

    public MethodSpec buildE() {
        return builderE.build();

    }
    public MethodSpec buildV() {
        return builderV.build();
